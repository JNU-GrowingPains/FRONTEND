...
- **재구매율 분석**: 전체 또는 선택한 상품들의 재구매율 통계
  - 단일 상품 선택: 해당 상품의 재구매 패턴 분석
  - 복수 상품 선택: 교차 재구매 포함한 평균 KPI 계산
  - 상품 미선택: 전체 상품 평균 KPI 표시
- **비회원 지원**: 회원과 비회원 모두 포함한 재구매 분석
  - 비회원 식별: 이름 + 주소 조합으로 동일인 판단
  - 회원/비회원 구분 표시 (등급 필드)
- **고객별 재구매 분석**: 개별 고객의 재구매 상품 및 배송지 분석
  - 고객 클릭 시 상세 정보 로드 (최대 60초)
  - 실시간 API 연동
- **주요 지표 (KPI)**:
  - 총 재구매 고객 수
  - 평균 재구매율 (%)
  ...
  - 재구매 고객 매출 기여도 (%)
- **시각화 차트**:
  - 재구매 고객 테이블 (회원/비회원 통합)
  - 고객별 재구매 상품 차트 (가로 막대그래프)
  - 지역별 재구매 배송지 차트 (도넛 차트)
